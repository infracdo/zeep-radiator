...
def extract_memory_limits(df, timestamp):
    df_filtered = df[
        (df["resource"] == "limits.memory") &
        (df["type"] == "hard")
    ]
...
def insert_memory_limits_to_postgres(df):
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS namespace_memory_quota_daily (
        ...
        for _, row in df.iterrows():
            cursor.execute("""
                INSERT INTO namespace_memory_quota_daily (namespace, memory_limit, collected_at)
                VALUES (%s, %s, %s)
                ON CONFLICT (namespace, collected_at) DO UPDATE
                SET memory_limit = EXCLUDED.memory_limit;
            """, (row["namespace"], row["value"], row["collected_at"]))

        conn.commit()
        cursor.close()
        conn.close()
        print("✅ Memory limits inserted into PostgreSQL.")
...
if __name__ == "__main__":
    ...
    df_memory_limits = extract_memory_limits(df, timestamp)

    print("\n🧾 Extracted Memory quota per namespace:")
    print(df_memory_limits)

    insert_memory_limits_to_postgres(df_memory_limits)
