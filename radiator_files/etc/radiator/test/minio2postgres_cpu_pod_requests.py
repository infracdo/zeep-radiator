...
PARQUET_PREFIX = "files/default/metrics/kube_pod_container_resource_requests/"
...
def insert_to_postgres(df):
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS pod_resource_usage_requests (
        ...
        for _, row in df.iterrows():
            cursor.execute("""
                INSERT INTO pod_resource_usage_requests (namespace, pod, resource, value, collected_at)
                VALUES (%s, %s, %s, %s, %s)
